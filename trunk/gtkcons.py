...
import sys, string, traceback, os
import pygtk
pygtk.require('2.0')
import gobject
import gtk, gtksourceview, pango, commands
...
class gtkoutfile:
    '''A fake output file object.  It sends output to a GTK SourceView widget,
...
class Console(gtk.VBox):
    def __init__(self, namespace={}, copyright='', quit_cb=None):
        ...
        self.text.set_show_line_markers(True)
    ...
    def eval(self):
        ...
        self.buffer.insert(iter, self.prompt.get() + l)
    ...
    def run_file(self, file):
        ...
        try:
            try:
                #~ r = eval("execfile('" + file + "')", self.namespace, self.namespace)
                #~ r = eval("execfile('" + file + "')")
                ...
                #r = commands.getoutput( c + 'python ' +  file + ' 2>&1')
                r = ""
		sys.stdout = os.popen('{ ' + c + ' python ' + file + '; } 2>&1', 'r')
		return
